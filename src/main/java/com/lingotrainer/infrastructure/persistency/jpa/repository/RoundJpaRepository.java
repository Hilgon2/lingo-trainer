...
public interface RoundJpaRepository extends JpaRepository<RoundEntity, Integer> {
    @Query(value = "SELECT u FROM #{#entityName} u WHERE u.game.id = :gameId AND u.active = true")
    RoundEntity findCurrentRound(@Param("gameId") int gameId);

    @Query(value = "SELECT u FROM #{#entityName} u "
            + "WHERE u.game.id = :gameId AND u.id = (SELECT MAX(ul.id) FROM #{#entityName} ul WHERE ul.active = false)")
